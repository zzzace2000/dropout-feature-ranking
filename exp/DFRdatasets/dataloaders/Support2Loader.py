...
class Support2Loader(MLPLoaderBase):
    '''
    ...
    def __init__(self, mode='classification', **kwargs):
        kwargs['mode'] = mode
        super(Support2Loader, self).__init__(**kwargs)

    def init_hyperamaters(self):
        return {
            'dimensions': [42, 170, 170, 2],
            'epochs': 600,
            'epoch_print': 1,
            'weight_lr': 1e-3,
            'lookahead': 12,
    ...
    def _load_data(self, testfold=4):
        x = pd.read_csv('data/support2/x_new.csv').values
        y = pd.read_csv('data/support2/y_new.csv').values.ravel()
...
class RegSupport2Loader(Support2Loader):
    def __init__(self, mode='regression', **kwargs):
        kwargs['mode'] = mode
        super(RegSupport2Loader, self).__init__(**kwargs)

    def init_hyperamaters(self):
        return {
            'dimensions': [42, 170, 170, 1],
            'epochs': 600,
            'epoch_print': 1,
            'weight_lr': 1e-3,
            'lookahead': 12,
    ...
    def _load_data(self, testfold=4):
        x = pd.read_csv('data/support2/x_new.csv').values
        y = pd.read_csv('data/support2/y_new_slos.csv').values.reshape((-1, 1))
        x = x.astype(np.float32)
